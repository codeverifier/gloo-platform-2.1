...
Provision() {
    echo "------------------------------------------------------------"
    echo "Injecting external service traffic management in west cluster"
    echo "Note: This will extend the mesh to an external service running on cloud run"
    echo "------------------------------------------------------------"
    echo ""
    ...
    envsubst < <(cat $DIR/backend-apis-team/ratings-route-table.yaml) | kubectl --context ${WEST_CONTEXT} apply -f -
    kubectl --context ${WEST_CONTEXT} apply -f $DIR/backend-apis-team/reviews-v5-external-endpoint.yaml
    kubectl --context ${WEST_CONTEXT} apply -f $DIR/backend-apis-team/reviews-v5-external-service.yaml
}
...
Delete() {
    ...
    envsubst < <(cat $DIR/backend-apis-team/ratings-route-table.yaml) | kubectl --context ${WEST_CONTEXT} delete -f -
    kubectl --context ${WEST_CONTEXT} delete -f $DIR/backend-apis-team/reviews-v5-external-endpoint.yaml
    kubectl --context ${WEST_CONTEXT} delete -f $DIR/backend-apis-team/reviews-v5-external-service.yaml
}
