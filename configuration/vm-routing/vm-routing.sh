...
Provision() {
    echo "------------------------------------------------------------"
    echo "Injecting VM traffic routing in west cluster"
    echo "Note: This will load balance traffic to ratings services in"
    echo "both clusters and the VM"
    echo "------------------------------------------------------------"
    echo ""
    ...
    # ---- Backend team ----
    envsubst < <(cat $DIR/backend-apis-team/workspace-settings.yaml) | kubectl --context ${WEST_CONTEXT} apply -f -
    kubectl --context ${WEST_CONTEXT} apply -f $DIR/backend-apis-team/ratings-route-table.yaml
    envsubst < <(cat $DIR/backend-apis-team/ratings-virtual-destination.yaml) | kubectl --context ${WEST_CONTEXT} apply -f -
...
Delete() {
    ...
    kubectl --context ${WEST_CONTEXT} delete -f $DIR/backend-apis-team/ratings-route-table.yaml
    envsubst < <(cat $DIR/backend-apis-team/ratings-virtual-destination.yaml) | kubectl --context ${WEST_CONTEXT} delete -f -
