...
Provision() {
    echo "------------------------------------------------------------"
    echo "Injecting cross cluster traffic shifting configuration in west cluster"
    echo "Note: This will route all the traffic to reviews to review version v3"
    echo "in east cluster"
    echo "------------------------------------------------------------"
    echo ""
    ...
    kubectl --context ${WEST_CONTEXT} apply -f $DIR/backend-apis-team/reviews-v3-route-table.yaml
    envsubst < <(cat $DIR/backend-apis-team/reviews-v3-virtual-destination.yaml) | kubectl --context ${WEST_CONTEXT} apply -f -
}
...
Delete() {
    ...
    kubectl --context ${WEST_CONTEXT} delete -f $DIR/backend-apis-team/reviews-v3-route-table.yaml
    envsubst < <(cat $DIR/backend-apis-team/reviews-v3-virtual-destination.yaml) | kubectl --context ${WEST_CONTEXT} delete -f -
}
